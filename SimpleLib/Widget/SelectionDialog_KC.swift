...
open class SelectionDialog_KC: UIView {
    ...
    fileprivate func setObservers() {
        NotificationCenter.default.addObserver(self, selector: #selector(SelectionDialog_KC.deviceOrientationDidChange(_:)), name: UIDevice.orientationDidChangeNotification, object: nil)
    }
    ...
    fileprivate func createContainerView() -> UIScrollView {
        ...
        /// Add ScrollView
        //let scrollView = UIScrollView(frame: CGRect(x: 0, y: titleHeight, width: widthDialog, height: minHeight))
        //scrollView.contentSize.height = CGFloat(items.count*40)
    ...
    fileprivate func createCloseButton() -> UIButton {
        ...
        button.addTarget(self, action: #selector(SelectionDialog_KC.close), for: UIControl.Event.touchUpInside)
