...
open class KCSelectionDialog: UIView {
    ...
    open func addItem(item itemTitle: String, didTapHandler: (() -> Void)) {
        let item = KCSelectionDialogItem(item: itemTitle, didTapHandler: didTapHandler)
        items.append(item)
    }
    
    open func addItem(item itemTitle: String, icon: UIImage, didTapHandler: (() -> Void)) {
    ...
    fileprivate func createContainerView() -> UIScrollView {
        ...
        for (index, item) in items.enumerated() {
            ...
            let itemTitleLabel = UILabel(frame: CGRect(x: 10, y: 0, width: 255, height: 40))
