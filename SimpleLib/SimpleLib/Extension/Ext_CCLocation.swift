...
let LON_MERID_IERS: Double = (3 * Double.pi / 180)
...
internal func latitudeISOFromLatitude(lat: Double, e: Double) -> Double {
    return log(tan(Double.pi / 4 + lat / 2) * pow((1 - e * sin(lat)) / (1 + e * sin(lat)), e / 2));
}

internal func latitudeFromLatitudeISO(lat_iso: Double, e: Double, eps: Double) -> Double {
    var phi_0: Double = 2 * atan(exp(lat_iso)) - Double.pi / 2
    var phi_i: Double = 2 * atan(pow((1 + e * sin(phi_0)) / (1 - e * sin(phi_0)), e / 2.0) * exp(lat_iso)) - Double.pi / 2
    var delta: Double = 1000
    while (delta > EPS) {
        phi_0 = phi_i
        phi_i = 2 * atan(pow((1 + e * sin(phi_0)) / (1 - e * sin(phi_0)), e / 2.0) * exp(lat_iso)) - Double.pi / 2
...
internal func lamberToGeographic(org: Point, zone: LambertZone, lon_merid: Double, e: Double, eps: Double) -> Point {
    ...
    return Point(x: lon, y: latitudeFromLatitudeISO(lat_iso: lat_iso, e: e, eps: eps), z: 0)
}
...
internal func geographicToCartesian(lon: Double, lat: Double, he: Double, a: Double, e: Double) -> Point {
    let N = lambertNormal(lat: lat, a: a, e: e)
...
internal func pointToWGS84(point: Point, zone: LambertZone) -> Point {
    var dest: Point
    if (.L93 == zone) {
        dest = lamberToGeographic(org: point, zone: zone, lon_merid: LON_MERID_IERS, e: E_WGS84, eps: EPS)
    } else {
        dest = lamberToGeographic(org: point, zone: zone, lon_merid: LON_MERID_PARIS, e: E_CLARK_IGN, eps: EPS)
        dest = geographicToCartesian(lon: dest.x, lat: dest.y, he: dest.z, a: A_CLARK_IGN, e: E_CLARK_IGN)
        dest.x -= 168
        dest.y -= 60
        dest.z += 320
        dest = cartesianToGeographic(org: dest, meridian: LON_MERID_GREENWICH, a: A_WGS84, e: E_WGS84, eps: EPS)
    }
    return dest
}
...
extension CLLocation {
    public convenience init(x: Double, y: Double, inZone: LambertZone) {
        ...
        var dest: Point = pointToWGS84(point: org, zone: inZone)
        let f: Double = 180.0 / Double.pi
        dest.x *= f
        dest.y *= f
    ...
    public convenience init (lat: Double, lon: Double, country: Int) {
        if (outOfChina(lat: lat, lon: lon) || country != 86) {
            MarsLat = lat
            MarsLon = lon
        } else {
            var dlat = tr_lat(x: lon - 105.0, y: lat - 35.0)
            var dlon = tr_lon(x: lon - 105.0, y: lat - 35.0)
...
extension CLLocationManager {
    ...
    public func WGSToGCJ(wgsLoc: CLLocationCoordinate2D) -> CLLocationCoordinate2D {
        ...
        if isLocationOutOfChina(location: wgsLoc) {
            var adjustLat = tr_lat(x: wgsLoc.longitude - 105.0, y: wgsLoc.latitude - 35.0)
            var adjustLon = tr_lon(x: wgsLoc.longitude - 105.0, y: wgsLoc.latitude - 35.0)
    ...
    public func isLocationOutOfChina(location: CLLocationCoordinate2D) -> Bool {
        return outOfChina(lat: location.longitude, lon: location.longitude)
    }
}
